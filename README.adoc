...
online repositories (e.g. https://search.maven.org/[Maven Central]) and allow other tasks (goals) to be performed. 
...
of the Apache Isis development effort. Using an archetype gives you the standard Maven folder structure, pom files and
...
and the process of building the Chat App is the same from the http://isis.apache.org/guides/ugfun/ugfun.html#__ugfun_getting-started_simpleapp-archetype_building-the-app[step 5.3]. 
...
Once you have the download zip archive extracted you should open the fold in a command prompt and do the
mvn clean install (first the clean and then the install goals) as in the SimpleApp documentation.
