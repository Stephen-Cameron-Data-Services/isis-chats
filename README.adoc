...
source from Github and build the Chats App sub-projects in the source using Maven, (4) Install database (MySQL with tools), and 
(5) Install a Java web-container (server), Apache Tomcat is used, (6) deploy the web-application archive (.war file), created in the 
...
constraints to be created automatically in the chats schema when the Chats App starts for the first time and connects to MySQL. On subsequent restarts
...
Another thing to do to ensure that with a Windows server shutdown and restart, that the MySQL and Tomcat services start in the correct order.
...
MySQL JDBC driver/connector from the location of the Chats specific libraries (extracted from chats.war), which is 
...
==== 7. Other installation issues

In restarting the server

==== Recovering existing database

The Chats MySQL database can be recovered from exports made on a nightly basis and saved to tape. The MySQL workbench can
do this import task to bring all the exported data
