...
@javax.jdo.annotations.Queries({
        @javax.jdo.annotations.Query(
                ...
                value = "SELECT "
                        + "FROM au.com.scds.chats.dom.module.note.NoteLinkableLink "
                        + "WHERE note == :note"),
        @javax.jdo.annotations.Query(
...
@javax.jdo.annotations.Unique(name="NoteLinkableLink_note_linkable_UNQ", members = {"note","linkableObjectType","linkableIdentifier"})
@DomainObject(
        objectType = "NoteLinkableLink"
)
public abstract class NoteLinkableLink extends PolymorphicAssociationLink<Note, NoteLinkable, NoteLinkableLink> {

    public static class InstantiateEvent
            extends PolymorphicAssociationLink.InstantiateEvent<Note, NoteLinkable, NoteLinkableLink> {

        public InstantiateEvent(final Object source, final Note aNote, final NoteLinkable linkable) {
            super(NoteLinkableLink.class, source, aNote, linkable);
        }
    }

    //region > constructor
    public NoteLinkableLink() {
    ...
    public static class Functions {
        public static Function<NoteLinkableLink, Note> GET_CASE = new Function<NoteLinkableLink, Note>() {
            @Override
            public Note apply(final NoteLinkableLink input) {
                return input.getSubject();
            }
        };
        public static Function<NoteLinkableLink, NoteLinkable> GET_CONTENT = new Function<NoteLinkableLink, NoteLinkable>() {
            @Override
            public NoteLinkable apply(final NoteLinkableLink input) {
