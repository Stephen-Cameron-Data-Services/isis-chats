...
import au.com.scds.chats.dom.report.view.DexNorthWest;
...
public class DEXNorthWestReport {
	...
	private DEXNorthWestReport() {
	}

	public DEXNorthWestReport(RepositoryService repository, IsisJdoSupport isisJdoSupport,
	...
	public DEXFileUpload build() throws Exception {
		...
		// DexNorth
		List<DexNorthWest> attendances = repository.allMatches(new QueryDefault(
				DexNorthWest.class, "allInPeriod", "startDateTime",
				this.startDateTime /* new DateTime("2016-01-13") */, "endDateTime",
				this.endDateTime));
		for (DexNorthWest attend : attendances) {
	...
	private Session buildNewSession(String sessionKey, DexNorthWest attend) {
	...
	private Case buildNewCase(DexNorthWest a) {
		Case case_ = new Case();
