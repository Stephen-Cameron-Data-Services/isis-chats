...
import au.com.scds.chats.dom.report.dex.DEXNorthReport.ClientIdGenerationMode;
...
public class DEXSouthReport {
	...
	private DEXSouthReport() {
	}

	public DEXSouthReport(RepositoryService repository, IsisJdoSupport isisJdoSupport, Participants participants,
			Integer year, Integer month, String regionName) {
	...
	public DEXFileUpload build() throws Exception {
		...
		// DexNorth
		List<DexSouth> attendances = repository
				.allMatches(new QueryDefault(DexSouth.class, "allInPeriod", "startDateTime",
						this.startDateTime /* new DateTime("2016-01-13") */, "endDateTime", this.endDateTime));
		for (DexSouth attend : attendances) {
			if (true /*
				...
				// System.out.print(attend.getAbbreviatedname() + "," +
				// attend.getInteractiondate() + ",");
				// System.out.print(attend. + "," + attend.getFirstName() + ","
				// + attend.getBirthDate() + ",");
				// System.out.println(attend.getMinutesAttended());
				switch (this.mode) {
				case NAME_KEY:
					clientKey = participant.getPerson().getFirstname().trim() + "_"
							+ participant.getPerson().getSurname().trim() + "_"
				...
				if (attend.getAbbreviatedname().equals("ChatsSocialCall")) {
					sessionKey = sessionKey = "To_" + clientKey + "_on_"
							+ attend.getInteractiondate().toString("dd-MM-YYYY");

				} else {
					sessionKey = attend.getAbbreviatedname().trim() + attend.getInteractiondate().toString("YYYYMMDD");
				}
	...
	private Session buildNewSession(String sessionKey, DexSouth attend) {
	...
	private Case buildNewCase(DexSouth attend) {
		Case case_ = new Case();
		case_.setCaseClients(new CaseClients());
		case_.setCaseId(createCaseId(attend.getAbbreviatedname()));
