 ...
 *  under the License.
 */
package au.com.scds.isis.viewer.wicket.ui.components.entity.properties;
...
public class MyEntityPropertiesForm extends FormAbstract<ObjectAdapter>
	...
	private static final String ID_TABS = "entityTabs";
	...
	public MyEntityPropertiesForm(final String id,
			final EntityModel entityModel, final Component owningPanel) {

		super(id, entityModel);
		this.owningPanel = owningPanel; // for repainting

		// SC 23/07/2015 added special case for Participant
		if ("Participant".equals(entityModel.getTypeOfSpecification()
				.getShortIdentifier())) {
			//TODO buildParticipantTabsGui();
			buildGui();
		} else {
			buildGui();
		}
		...
		final String concurrencyExceptionIfAny = entityModel
				.getAndClearConcurrencyExceptionIfAny();
	...
	/**
	 * 
	 * SC use tabs for Participant Entity
	 */
	private void buildParticipantTabsGui() {

		final EntityModel entityModel = (EntityModel) getModel();
		final ColumnSpans columnSpans = entityModel.getObject()
		...
		if (columnSpans.getLeft() > 0) {
			addedProperties = addPropertiesInColumn(leftColumn,
					MemberGroupLayoutHint.LEFT, columnSpans);
		...
		if (!addedProperties) {
			// a bit hacky...
			Components.permanentlyHide(this, ID_EDIT_BUTTON, ID_OK_BUTTON,
					ID_CANCEL_BUTTON, ID_FEEDBACK);
		}

		// middle column
		/*if (columnSpans.getMiddle() > 0) {
			MarkupContainer middleColumn = new WebMarkupContainer(
					ID_MIDDLE_COLUMN);
			add(middleColumn);
			addPropertiesInColumn(middleColumn, MemberGroupLayoutHint.MIDDLE,
					columnSpans);
		} else {
		...
		/*if (columnSpans.getRight() > 0) {
			MarkupContainer rightColumn = new WebMarkupContainer(
					ID_RIGHT_COLUMN);
			add(rightColumn);
			addPropertiesInColumn(rightColumn, MemberGroupLayoutHint.RIGHT,
					columnSpans);
		} else {
		...
		/*if (columnSpans.getCollections() > 0) {
			...
			final Component collectionsColumn = getComponentFactoryRegistry()
					.addOrReplaceComponent(this, idCollectionsToShow,
	    ...
	    tabs.add(new AbstractTab(new Model<String>("first tab")) {
	        public Panel getPanel(String panelId) { return new TabPanel(panelId); }
	    });
	    ...
	    add(new TabbedPanel(ID_TABS, tabs)*/
	...
	private void buildGui() {
		...
		Components.permanentlyHide(this, ID_TABS);

	}

	private boolean addPropertiesInColumn(MarkupContainer markupContainer,
			MemberGroupLayoutHint hint, ColumnSpans columnSpans) {
		...
		final List<ObjectAssociation> associations = visibleProperties(adapter,
				objSpec, Where.OBJECT_FORMS);
...
//TODO remove		System.out.println(">>>" + groupNames.toString());
		...
		for (String groupName : groupNames) {
			...
			final WebMarkupContainer memberGroupRvContainer = new WebMarkupContainer(
					memberGroupRv.newChildId());
			...
			for (final ObjectAssociation association : associationsInGroup) {
				...
				addPropertyToForm(entityModel,
						(OneToOneAssociation) association, propertyRvContainer,
						memberGroupActions);
			}

			final List<LinkAndLabel> actionsPanel = LinkAndLabel.positioned(
					memberGroupActions, ActionLayout.Position.PANEL);
			...
			AdditionalLinksPanel.addAdditionalLinks(memberGroupRvContainer,
					ID_ASSOCIATED_ACTION_LINKS_PANEL, actionsPanel,
	...
	private void addPropertyToForm(final EntityModel entityModel,
		...
		final List<ObjectAction> associatedActions = EntityActionUtil
				.getObjectActionsForAssociation(entityModel, otoa,
						getDeploymentType());

		entityActions.addAll(EntityActionUtil
				.asLinkAndLabelsForAdditionalLinksPanel(entityModel,
						associatedActions));
	...
	private List<ObjectAssociation> visibleProperties(
			final ObjectAdapter adapter, final ObjectSpecification objSpec,
			Where where) {
	...
	abstract class AjaxButtonWithOnError extends AjaxButton {
		...
		/**
		 * Render the 'type' attribute even for invisible buttons to avoid <a
		 * href="https://github.com/twbs/bootlint/wiki/W007">Bootlint W007</a>
		 * 
		 * @param tag
		...
		protected void renderPlaceholderTag(ComponentTag tag, Response response) {
			String ns = Strings.isEmpty(tag.getNamespace()) ? null : tag
					.getNamespace() + ':';
	...
	public class AjaxButtonForValidate extends AjaxButtonWithOnError implements
		...
		public String preValidate() {
			...
			// exceptions
			try {
				getEntityModel().load(ConcurrencyChecking.CHECK); // could have
																	// also just
																	// called
																	// #getObject(),
																	// since
		...
		public void validate() {
			...
			MyEntityPropertiesForm form = MyEntityPropertiesForm.this;
			...
			if (preValidationErrorIfAny != null) {
				...
				final EntityPage entityPage = new EntityPage(
						MyEntityPropertiesForm.this.getModelObject(), null);
				MyEntityPropertiesForm.this.setResponsePage(entityPage);
			} else {
		...
		protected void onSubmit(AjaxRequestTarget target, Form<?> form) {
			...
			doPreApply();
			if (applyFormChangesElse(target))
				return;
			final Object redirectIfAny = doPostApply();

			if (flushChangesElse(target))
				return;
			...
			//
			// it's necessary to zap the page parameters (so mapping is to just
			// wicket/page?nn)
			...
			if (redirectIfAny != null) {
				objectAdapter = getPersistenceSession().getAdapterManager()
						.adapterFor(redirectIfAny);
			} else {
				...
				objectAdapter = getEntityModel().getObjectAdapterMemento()
	...
	private void addButtons(MarkupContainer markupContainer) {
		...
		// edit button
		editButton = new AjaxButtonWithOnError(ID_EDIT_BUTTON,
				new ResourceModel("editLabel")) {
			private static final long serialVersionUID = 1L;
			...
			public void validate() {
				...
				try {
					...
					getMessageBroker().addMessage(
							"Object changed by "
									+ ex.getOid().getVersion().getUser()
			...
			protected void updateAjaxAttributes(AjaxRequestAttributes attributes) {
				...
				attributes
						...
						.add(new org.apache.wicket.ajax.attributes.AjaxCallListener() {

							private static final long serialVersionUID = 1L;

							@Override
							public CharSequence getSuccessHandler(
									Component component) {
		...
		// ok button
		okButton = new AjaxButtonForValidate(ID_OK_BUTTON, new ResourceModel(
				"okLabel"));
		markupContainer.add(okButton);

		// cancel button
		cancelButton = new AjaxButtonForCancel(ID_CANCEL_BUTTON,
			...
			protected void onSubmit(final AjaxRequestTarget target,
				...
				getForm().clearInput();
				getForm().visitFormComponentsPostOrder(
	...
	private boolean applyFormChangesElse(AjaxRequestTarget target) {
		...
		final String invalidReasonIfAny = getEntityModel()
				.getReasonInvalidIfAny();
	...
	private String recognizeExceptionAndNotify(RuntimeException ex,
	...
	private void feedbackOrNotifyAnyRecognizedError(
			String recognizedErrorMessageIfAny, Component feedbackComponentIfAny) {
	...
	private String recognizeException(RuntimeException ex) {
		...
		final String message = new ExceptionRecognizerComposite(
				exceptionRecognizers).recognize(ex);
		return message;
	}
	...
	private boolean isAnythingEditable() {
		...
		return !enabledAssociations(adapter, adapter.getSpecification())
				.isEmpty();
	}

	private List<ObjectAssociation> enabledAssociations(
			final ObjectAdapter adapter, final ObjectSpecification objSpec) {
	...
	@SuppressWarnings("unchecked")
	private Filter<ObjectAssociation> enabledAssociationFilter(
			final ObjectAdapter adapter) {
		return Filters.and(ObjectAssociation.Filters.PROPERTIES,
				ObjectAssociation.Filters.enabled(getAuthenticationSession(),
						adapter, Where.OBJECT_FORMS));
	}
	...
	private void addFeedbackGui(final MarkupContainer markupContainer) {
		feedback = new NotificationPanel(ID_FEEDBACK, this,
	...
	private static void addClassForSpan(final Component component,
			final int numGridCols) {
